...
def listening_announcements(phase,seed,band,support,task_id):
    ...
    if phase_mode(phase)=="transfer":
        return open_task(task_id,"eng.y2.listening.announcement.notes","Listen to the tutor reading the announcement. Write only the destination, time and platform as short notes.",band,support,params={"time":time,"platform":platform,"tutor_script":script},dimensions=["listening","note_taking","detail"],acceptable=f"Oxford; {time}; platform {platform}",rubric={"full_credit":"Captures all three practical details without a full transcription."},errors=[{"pattern":"transcribes_everything","code":"strategy_error"}])
...
def listening_narrative(phase,seed,band,support,task_id):
    ...
    if phase_mode(phase)=="transfer":
        return open_task(task_id,"eng.y2.listening.narrative.sequence","Listen to the tutor reading a short past experience. Note the place, two events in order and the speaker's final opinion.",band,support,params={"place":place,"tutor_script":script},dimensions=["listening","sequencing","gist_detail"],acceptable=f"{place}; arrival/lunch/home in order; best part being together",rubric={"full_credit":"Captures main setting, sequence and final evaluation."})
